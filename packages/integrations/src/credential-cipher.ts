...
export class CredentialCipher {
  ...
  encrypt(plaintext: string): string {
    ...
    return [PAYLOAD_VERSION, nonce.toString('base64url'), ciphertext.toString('base64url'), cipher.getAuthTag().toString('base64url')].join('.');
  }

  decrypt(payload: string): string {
    try {
      const [version, encodedNonce, encodedCiphertext, encodedTag, extra] = payload.split('.');
      if (version !== PAYLOAD_VERSION || !encodedNonce || encodedCiphertext === undefined || !encodedTag || extra !== undefined) throw new Error('Invalid payload format');
