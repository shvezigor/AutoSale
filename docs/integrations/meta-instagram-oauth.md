...
| Картка показує `META_DISCONNECT_CLEANUP_FAILED` | Локальне підключення вже вимкнене, а точний старий credential перенесено в чергу очищення; active connection більше не містить цей token. Увійдіть як `OWNER`, відновіть доступ до Meta й натисніть **Повторити очищення**. Сервіс окремо запам'ятовує прогрес unsubscribe і revoke, тому повторює лише незавершену дію. Нове OAuth-підключення заблоковане лише доки для tenant існує незавершене cleanup-завдання; після terminal completion reconnect розблоковується. |
| Картка показує `META_CLEANUP_PERMANENT_FAILURE` або після трьох невдалих повторів пропонує розблокування | Автоматичне очищення отримало постійну відмову від Meta або безпечний поріг повторних невідомих помилок. `OWNER` може повторити очищення або, якщо зовнішній доступ уже неможливо відновити, підтвердити **Розблокувати підключення**; підтвердження попереджає, що віддалене очищення не підтверджене, автоматичні повтори для цього credential буде припинено, а залишковий стан у Meta може зберегтися. Дія створює audit-подію, переводить cleanup-завдання в terminal dead-letter стан і розблоковує новий OAuth без повернення старого credential у connection. Невідомі помилки Meta не вважаються успіхом; до порогу вони залишаються retryable. |
