...
| Consistent field validation UX across forms | Validation pending | [`field validation UX`](../superpowers/specs/2026-09-21-field-validation-ux-design.md), [`acceptance`](../acceptance/field-validation-checklist.md) | Shared field controls and safe API issues cover auth, profile, team, catalogue, order, payment, delivery, supplier chat, Instagram reply and public demo. Google picker forms retain integration-level failures because tabs are provider-selected. Public-demo, registration and partial authenticated smoke (profile, catalogue, bank account) pass; payment/TTN live checks remain. |
...
| Meta Instagram OAuth, webhook ingestion and inbox | Validation pending | [`Instagram system design`](../superpowers/specs/2026-08-26-instagram-order-capture-design.md), [`Meta access`](../integrations/meta-access.md), [`OAuth runbook`](../integrations/meta-instagram-oauth.md), [`MVP acceptance`](../acceptance/mvp-checklist.md) | `apps/api/src/meta`, `apps/api/src/integrations/instagram-*`, `apps/worker/src/instagram`, `apps/api/src/conversations`, `apps/web/src/components/message-thread.tsx`; shared posts, reels, stories, textual HTTP(S) links and unknown attachments retain a visible safe representation, including idempotent recovery of legacy empty messages. |
...
| AI order recognition, catalogue-safe matching and optional conversational intent detection | Available | [`Instagram order capture design`](../superpowers/specs/2026-08-26-instagram-order-capture-design.md), [`conversational intent design`](../superpowers/specs/2026-09-18-conversational-order-intent-detection-design.md) | `apps/worker/src/orders`, `packages/contracts/src/orders.ts`, `apps/api/src/orders`, `apps/web/src/components/order-settings-form.tsx`; conversational modes require explicit purchase intent in the new inbound anchor itself, not merely in older context. |
...
| Delivery adapters: Nova Poshta, Meest and Ukrposhta | Validation pending | [`delivery design`](../superpowers/specs/2026-09-10-delivery-carriers-and-nova-poshta-design.md), [`Ukrposhta research`](../research/ukrposhta-api-integration-research.md), [`MVP acceptance`](../acceptance/mvp-checklist.md) | `apps/api/src/delivery`, `apps/worker/src/delivery`, `packages/integrations/src/{nova-poshta,meest,ukrposhta}*`, `apps/web/src/components/shipment-review-dialog{,.spec}.tsx`; UI and API both gate creation on a successful quote, while a live TTN lifecycle check remains. |
| Order commercial terms, legal entities, bank accounts and audited payment facts | Available | [`commercial terms and bank accounts design`](../superpowers/specs/2026-09-19-order-commercial-terms-and-bank-accounts-design.md), [`payment facts design`](../superpowers/specs/2026-09-20-order-payment-facts-design.md), [`commercial acceptance`](../acceptance/order-commercial-terms-checklist.md), [`payment acceptance`](../acceptance/order-payment-facts-checklist.md) | `packages/database/src/{commercial-terms,order-payments}.ts`, `apps/api/src/commercial-settings`, `apps/api/src/orders/{commercial-terms,payments}*`, `apps/web/src/components/{commercial-settings-hub,order-review-panel,order-commercial-terms-card,order-payments-card}.tsx`, `tests/e2e/order-payment-facts.spec.ts`; owner-only confirmed deletion is available for unused entities/accounts, while historically referenced records require deactivation; initial legacy calculation refreshes payment entry without a page reload; personal Telegram alerts do not lock corrections, unlike supplier dispatch; manual facts are available, while the remaining live acceptance, automated reconciliation and refunds are deliberately deferred to a combined backlog phase. |
...
| Ukrainian/English workspace localization and Sales AITO customer-facing naming | Available | [`localization design`](../superpowers/specs/2026-09-14-user-profile-localization-table-sorting-design.md) | `apps/web/src/i18n`, locale-aware components, auth/workspace brand surfaces and route tests |
