...
| Consistent field validation UX across forms | Validation pending | [`field validation UX`](../superpowers/specs/2026-09-21-field-validation-ux-design.md), [`acceptance`](../acceptance/field-validation-checklist.md) | Shared field controls and safe API issues cover auth, profile, team, catalogue, order, payment, delivery, supplier chat, Instagram reply and public demo. Google picker forms retain integration-level failures because tabs are provider-selected. Full automated suite plus public-demo and registration browser checks pass; authenticated production browser smoke remains. |
