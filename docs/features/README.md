...
| Order commercial terms, legal entities and bank accounts | Available | [`commercial terms and bank accounts design`](../superpowers/specs/2026-09-19-order-commercial-terms-and-bank-accounts-design.md), [`acceptance checklist`](../acceptance/order-commercial-terms-checklist.md) | `packages/database/src/commercial-terms.ts`, `apps/api/src/commercial-settings`, `apps/api/src/orders/commercial-terms*`, `apps/worker/src/orders/triggered-order.processor.ts`, `apps/web/src/components/{commercial-settings-hub,order-commercial-terms-card}.tsx`; actual payment ledger and reconciliation remain a separate planned lifecycle. |
