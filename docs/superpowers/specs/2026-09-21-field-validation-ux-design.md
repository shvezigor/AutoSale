...
Реалізовано спільний `FormField`/`FieldError`, локалізовані client-side обмеження, фокус першого некоректного поля й контракт безпечних API `issues`. Мігровано автентифікацію, onboarding, профіль, команду, редактор товарів, імпорт каталогу, юридичні особи, банківські рахунки, виправлення замовлення, комерційні умови, факти оплат, доставку, ТТН, вибір чату постачальника, Instagram-відповіді та публічну демозаявку. Після виправлення конкретного поля його помилка зникає без очищення інших.
...
Репозиторний контракт інвентаризує всі production-форми та допускає лише два задокументовані non-field винятки для Google Picker. Повний набір автоматизованих тестів і браузерний тест демоформи на чотирьох ширинах пройшли. Частковий авторизований production browser smoke (профіль, новий товар, банківський рахунок) і решта неперевірених сценаріїв записуються окремо в [приймальному чеклисті](../../acceptance/field-validation-checklist.md). На вузькому планшетному екрані неактивний placeholder пошуку в шапці приховується, щоб не виштовхувати дії профілю за межі viewport.
