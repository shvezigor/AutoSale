...
While a message is `PENDING`, `SENDING`, or newly `UNKNOWN`, the conversation client polls only that conversation about every two seconds. Polling stops when no transient message remains or after a 60-second bound. A successful message shows `Надіслано`. A terminal failure that is safe to retry shows `Не вдалося надіслати` and a `Повторити` action. A network timeout or ambiguous Meta `5xx` result shows `Статус доставки невідомий` without automatic retry, because Meta does not document a client idempotency key for the Send API. AutoSale waits within that polling window for a matching echo that can still reconcile this state to `SENT`.
...
- `FAILED`: Meta explicitly rejected the attempt and a controlled retry can be offered;
- `UNKNOWN`: the request may have reached Meta, so automatic retry is prohibited until an echo resolves it.

Message records gain nullable outbound-only fields:

- `clientIdempotencyKey` — unique inside the tenant;
- `sentByUserId` — the tenant member who initiated the message;
...
- permits only a tenant-bound outbound message in `FAILED` with a safe retry code and an active connection;
...
Retry attempts update the same row. The client idempotency key protects browser retries; provider and webhook uniqueness protect callback replay. It cannot provide provider-side idempotency, so an ambiguous delivery is held in `UNKNOWN` until a matching echo arrives and is never retried automatically.
