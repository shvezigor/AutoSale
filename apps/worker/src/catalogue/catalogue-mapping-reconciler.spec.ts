...
describe('CatalogueMappingReconciler', () => {
  it('durably dispatches an uploaded run when the API enqueue never happened', async () => {
    const findMany = vi.fn().mockResolvedValue([{ id: 'run-1', tenantId: 'tenant-1' }]);
    ...
    expect(findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ OR: expect.arrayContaining([expect.objectContaining({ status: 'UPLOADED' }), expect.objectContaining({ status: 'MAPPING', updatedAt: { lt: expect.any(Date) } })]) }) }));
  });

  it('leaves durable candidates for a later retry when queueing fails', async () => {
    const findMany = vi.fn().mockResolvedValue([{ id: 'run-2', tenantId: 'tenant-2' }]);
