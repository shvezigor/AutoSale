...
import type { OrderPaymentStatus, OrderPaymentSummary } from '../../../../packages/contracts/src/payments';
...
type OrdersTableProps = { orders: ManagerOrder[]; page: number; pageSize: number; total: number; search?: string; status?: OrderStatus; procurementStatus?: ProcurementSummary; shipmentStatus?: ShipmentStatus; paymentStatus?: OrderPaymentStatus; sort?: OrderSort; direction?: SortDirection };

export function OrdersTable({ orders, page, pageSize, total, search = '', status, procurementStatus, shipmentStatus, paymentStatus, sort = 'date', direction = 'desc' }: OrdersTableProps) {
  const { t } = useI18n();
  ...
  const [selectedPayment, setSelectedPayment] = useState<OrderPaymentStatus | ''>(paymentStatus ?? '');
  const navigate = (nextPage: number, nextPageSize = pageSize, nextStatus = selectedStatus, nextProcurement = selectedProcurement, nextShipment = selectedShipment, nextPayment = selectedPayment, nextSort = sort, nextDirection = direction) => router.replace(ordersUrl(query, nextStatus, nextProcurement, nextShipment, nextPayment, nextPage, nextPageSize, nextSort, nextDirection), { scroll: false });
  ...
  const changeSort = (nextSort: OrderSort) => navigate(1, pageSize, selectedStatus, selectedProcurement, selectedShipment, selectedPayment, nextSort, nextSort === sort ? (direction === 'asc' ? 'desc' : 'asc') : naturalDirection(nextSort));
  const returnTo = ordersUrl(search, status ?? '', procurementStatus ?? '', shipmentStatus ?? '', paymentStatus ?? '', page, pageSize, sort, direction);
  ...
  const paymentStatuses: Array<{ value: OrderPaymentStatus | ''; label: string }> = [
    { value: '', label: t('orders.allPaymentStatuses') }, { value: 'UNPAID', label: t('orders.paymentUnpaid') }, { value: 'PARTIALLY_PAID', label: t('orders.paymentPartial') }, { value: 'PAID', label: t('orders.paymentPaid') }, { value: 'OVERPAID', label: t('orders.paymentOverpaid') },
  ];

  return <>
    <div className="orders-toolbar">
      <form onSubmit={submitSearch} role="search" data-validation-context="non-field"><label className="sr-only" htmlFor="orders-search">{t('orders.searchLabel')}</label><input id="orders-search" onChange={(event) => setQuery(event.target.value)} placeholder={t('orders.searchPlaceholder')} type="search" value={query} /><button className="secondary-button" type="submit">{t('orders.search')}</button></form>
      ...
      <label className="orders-status-filter"><span className="sr-only">{t('orders.paymentStatus')}</span><select aria-label={t('orders.paymentStatus')} onChange={(event) => { const value = event.target.value as OrderPaymentStatus | ''; setSelectedPayment(value); navigate(1, pageSize, selectedStatus, selectedProcurement, selectedShipment, value); }} value={selectedPayment}>{paymentStatuses.map((item) => <option key={item.value || 'all'} value={item.value}>{item.label}</option>)}</select></label>
    </div>
...
function OrderRow({ order, href, onOpen, rowNumber }: { order: ManagerOrder; href: string; onOpen: (href: string) => void; rowNumber: number }) {
  ...
  return <tr className="orders-table-row" onClick={() => onOpen(href)}><td className="table-row-index">{formatNumber(rowNumber)}</td><td className="order-public-number">{order.publicNumber}</td><td><strong>{productLabel(order, t)}</strong><small>{productMeta(order, t, formatNumber)}</small></td><td><strong>{customerLabel(order, t)}</strong><small>{contactLabel(order, t)}</small></td><td><span className="order-delivery">{deliveryLabel(order, t)}</span></td><td><span className="order-delivery">{shipmentLabel(order, t)}</span></td><td><OrderStatusBadge status={order.status} /></td><td><PaymentStatusBadge summary={order.paymentSummary} /></td><td><ProcurementBadge status={order.procurementSummary} /></td><td className="order-confidence">{confidenceLabel(order, formatNumber)}</td><td className="orders-date-column"><time dateTime={order.createdAt}>{dateLabel(order.createdAt, formatDate)}</time></td><td><Link aria-label={viewLabel(order, t)} className="text-button" href={href} onClick={(event: MouseEvent<HTMLAnchorElement>) => event.stopPropagation()}>{t('orders.view')}</Link></td></tr>;
}

function OrderCard({ order, href, rowNumber }: { order: ManagerOrder; href: string; rowNumber: number }) {
  const { t, formatDate, formatNumber } = useI18n();
  return <Link aria-label={viewLabel(order, t)} className="orders-card" href={href}><div className="orders-card-heading"><span><span className="orders-card-index">{t('orders.number')} {formatNumber(rowNumber)} · {order.publicNumber}</span><strong>{productLabel(order, t)}</strong><small>{productMeta(order, t, formatNumber)}</small></span><span className="order-confidence">{confidenceLabel(order, formatNumber)}</span></div><dl><div><dt>{t('orders.customer')}</dt><dd>{customerLabel(order, t)}</dd></div><div><dt>{t('orders.delivery')}</dt><dd>{deliveryLabel(order, t)}</dd></div><div><dt>{t('orders.shipment')}</dt><dd>{shipmentLabel(order, t)}</dd></div><div><dt>{t('orders.payment')}</dt><dd><PaymentStatusBadge summary={order.paymentSummary} /></dd></div><div><dt>{t('orders.procurement')}</dt><dd><ProcurementBadge status={order.procurementSummary} /></dd></div><div><dt>{t('orders.date')}</dt><dd>{dateLabel(order.createdAt, formatDate)}</dd></div></dl><div className="orders-card-actions"><OrderStatusBadge status={order.status} /><span className="text-button">{t('orders.view')}</span></div></Link>;
}
...
function PaymentStatusBadge({ summary }: { summary: OrderPaymentSummary | null }) { const { t } = useI18n(); if (!summary) return <span aria-label={t('orders.paymentUnavailable')}>—</span>; return <span className={`payment-status payment-${summary.status.toLowerCase()}`}>{paymentStatusLabel(summary.status, t)}</span>; }
...
function paymentStatusLabel(status: OrderPaymentStatus, t: Translator) { return ({ UNPAID: t('orders.paymentUnpaid'), PARTIALLY_PAID: t('orders.paymentPartial'), PAID: t('orders.paymentPaid'), OVERPAID: t('orders.paymentOverpaid') } satisfies Record<OrderPaymentStatus, string>)[status]; }
function naturalDirection(sort: OrderSort): SortDirection { return sort === 'confidence' || sort === 'date' ? 'desc' : 'asc'; }
function ordersUrl(query: string, status: OrderStatus | '', procurementStatus: ProcurementSummary | '', shipmentStatus: ShipmentStatus | '', paymentStatus: OrderPaymentStatus | '', page: number, pageSize: number, sort: OrderSort, direction: SortDirection) { const params = new URLSearchParams(); if (query.trim()) params.set('search', query.trim()); if (status) params.set('status', status); if (procurementStatus) params.set('procurementStatus', procurementStatus); if (shipmentStatus) params.set('shipmentStatus', shipmentStatus); if (paymentStatus) params.set('paymentStatus', paymentStatus); if (sort !== 'date') params.set('sort', sort); if (direction !== naturalDirection(sort)) params.set('direction', direction); if (page > 1) params.set('page', String(page)); if (pageSize !== 25) params.set('pageSize', String(pageSize)); const value = params.toString(); return value ? `/orders?${value}` : '/orders'; }
