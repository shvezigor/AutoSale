...
export function OrdersTable({ orders, page, pageSize, total, search = '', status, procurementStatus, shipmentStatus }: OrdersTableProps) {
  ...
  return <>
    ...
    {orders.length === 0 ? <p className="orders-empty" role="status">{search || status || procurementStatus ? 'Замовлень за цим запитом не знайдено.' : 'Замовлень поки немає.'}</p> : <>
      <div className="orders-table-wrap"><table aria-label="Замовлення" className="orders-table"><thead><tr><th className="table-row-index" scope="col">№</th><th scope="col">Товар</th><th scope="col">Клієнт</th><th scope="col">Доставка</th><th scope="col">Відправлення</th><th scope="col">Статус</th><th scope="col">Комплектація</th><th scope="col">Впевненість</th><th className="orders-date-column" scope="col">Дата</th><th scope="col"><span className="sr-only">Дії</span></th></tr></thead><tbody>{orders.map((order, index) => <OrderRow href={orderDetailUrl(order.id, returnTo)} key={order.id} order={order} onOpen={(href) => router.push(href)} rowNumber={rowNumber(page, pageSize, index)} />)}</tbody></table></div>
...
function OrderRow({ order, href, onOpen, rowNumber }: { order: ManagerOrder; href: string; onOpen: (href: string) => void; rowNumber: number }) {
  return <tr className="orders-table-row" onClick={() => onOpen(href)}><td className="table-row-index">{rowNumber}</td><td><strong>{productLabel(order)}</strong><small>{productMeta(order)}</small></td><td><strong>{customerLabel(order)}</strong><small>{contactLabel(order)}</small></td><td><span className="order-delivery">{deliveryLabel(order)}</span></td><td><span className="order-delivery">{shipmentLabel(order)}</span></td><td><OrderStatusBadge status={order.status} /></td><td><ProcurementBadge status={order.procurementSummary} /></td><td className="order-confidence">{confidenceLabel(order)}</td><td className="orders-date-column"><time dateTime={order.createdAt}>{dateLabel(order.createdAt)}</time></td><td><Link aria-label={viewLabel(order)} className="text-button" href={href} onClick={(event: MouseEvent<HTMLAnchorElement>) => event.stopPropagation()}>Переглянути</Link></td></tr>;
}

function OrderCard({ order, href, rowNumber }: { order: ManagerOrder; href: string; rowNumber: number }) {
  return <Link aria-label={viewLabel(order)} className="orders-card" href={href}><div className="orders-card-heading"><span><span className="orders-card-index">№ {rowNumber}</span><strong>{productLabel(order)}</strong><small>{productMeta(order)}</small></span><span className="order-confidence">{confidenceLabel(order)}</span></div><dl><div><dt>Клієнт</dt><dd>{customerLabel(order)}</dd></div><div><dt>Доставка</dt><dd>{deliveryLabel(order)}</dd></div><div><dt>Відправлення</dt><dd>{shipmentLabel(order)}</dd></div><div><dt>Комплектація</dt><dd><ProcurementBadge status={order.procurementSummary} /></dd></div><div><dt>Дата</dt><dd>{dateLabel(order.createdAt)}</dd></div></dl><div className="orders-card-actions"><OrderStatusBadge status={order.status} /><span className="text-button">Переглянути</span></div></Link>;
}
...
function shipmentLabel(order: ManagerOrder) { if (!order.shipment) return 'Ще не створено'; return order.shipment.trackingNumber ? `${statusLabels[order.shipment.status] ?? order.shipment.status} · ${order.shipment.trackingNumber}` : statusLabels[order.shipment.status] ?? order.shipment.status; }
...
function dateLabel(value: string) { const parts = new Intl.DateTimeFormat('uk-UA', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hourCycle: 'h23', timeZone: 'Europe/Kyiv' }).formatToParts(new Date(value)); const part = (type: Intl.DateTimeFormatPartTypes) => parts.find((item) => item.type === type)?.value ?? ''; return `${part('day')}.${part('month')}.${part('year')}, ${part('hour')}:${part('minute')}`; }
