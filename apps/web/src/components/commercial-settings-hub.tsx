...
function FormActions({ editing, pending, onCancel, t }: { editing: string | null; pending: boolean; onCancel: () => void; t: T }) { return <div className="commercial-form-actions">{editing && <button type="button" className="button-secondary" onClick={onCancel}>{t('settings.cancel')}</button>}<button type="submit" className="button-primary" disabled={pending}>{pending ? t('settings.saving') : editing ? t('settings.save') : t('settings.add')}</button></div>; }
