...
export class GoogleSheetsSettingsService {
  constructor(private readonly prisma: PrismaClient, private readonly tenantId: string, private readonly sheets?: GoogleSheetsAdapter) {}

  async get() {
    const row = await this.prisma.googleSheetsDestination.findUnique({ where: { tenantId: this.tenantId } });
  ...
  async update(input: { spreadsheetId: string; sheetName: string }) {
    const row = await this.prisma.googleSheetsDestination.upsert({
      where: { tenantId: this.tenantId },
      create: { tenantId: this.tenantId, ...input, requiredHeaders, status: 'PENDING' },
  ...
  async validate() {
    ...
    try {
      ...
      await this.prisma.googleSheetsDestination.update({ where: { tenantId: this.tenantId }, data: { status, lastValidatedAt: new Date(), errorSummary: missingHeaders.length ? `Missing headers: ${missingHeaders.join(', ')}` : null } });
      ...
      await this.prisma.googleSheetsDestination.update({ where: { tenantId: this.tenantId }, data: { status: 'ERROR', lastValidatedAt: new Date(), errorSummary: summary } });
